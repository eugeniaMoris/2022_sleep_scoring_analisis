...
def train_data(wavelet, rus, level, features, trees, epochs, index):
    ...
    S2, SWS = multilabel_confusion_matrix(y_valid, y_predict)

    acc_s2, f1_s2 = metrics.get_metrics(S2,1)
    acc_sws, f1_sws = metrics.get_metrics(SWS,1)

    
    f = open("Acc_S2-SWS.txt", "w")
    f.write('Data: '+ '\n' + 
        'acc: '+ str(np.mean(acc)) + '\n' +
        'acc_s2 '+ str(np.mean(acc_s2))+ '\n' +
        'acc_sws '+ str(np.mean(acc_sws)))
    f.close()


    #print('acc: ', acc, ' f1: ', f1, ' pres: ', pres, ' rec: ', rec)
    return acc, acc_s2, acc_sws, f1, f1_s2, f1_sws

def save_results(f1, f1_s2, f1_sws, w, r, l, f,t):
    
    df = pd.read_csv("results_cascade_S2-sws.csv")
    f1= np.mean(f1)
    ...
    df[w + '_' + str(r) + '_' + str(l) + '_' + str(f)+'_' + str(t)] = [f1, f1_s2,f1_sws]

    df.to_csv("results_cascade_S2-sws.csv", index=False)
...
def train(wavelet, rus, level, features,trees, epochs, test_epochs):
    ...
    #ESTANDARIZACION ENTRENAMIEO
    train_norm, medias, stds = normalize_mat(train_matrix)
    np.save('medias_3.npy', medias)
    np.save('std_3.npy', stds)
    ...
    #guardo el modelo
    joblib.dump(clf1, "./ccascade_3.joblib")
    #with open('./cascade_s1-r-w_1ch', 'wb') as f:
    #        cPickle.dump(clf1, f)


    index = 0
    for t in test_epochs:
        (acc[index], acc_s2[index], acc_sws[index],
        f1[index], f1_s2[index], f1_sws[index]) = predecir_sj(wavelet,rus, level, features, clf1,t, medias,stds,pca)

        index = index +1

    f = open("Acc_cascade_3.txt", "w")
    f.write('Data: '+ 'acc: '+ str(np.mean(acc)) +'acc_s2 '+ str(np.mean(acc_s2))+'acc_sws '+ str(np.mean(acc_sws)))
    f.close()

    file = open("Fscore_cascade_3.txt", "w")
    file.write('Data: '+ 'fs_s2 '+ str(f1_s2)+'\n'
    + 'fs_sws '+ str(f1_sws)+'\n')
    file.close()

    save_results(f1, f1_s2, f1_sws, wavelet, rus, level, features, trees)
...
def predecir_sj(wavelet, rus, level, features, rf, test, medias, stds,pca=10):
    ...
    #5 CLASES
    S2, SWS = multilabel_confusion_matrix(y_test, y_predict)
...
def main(wavelet,RUS,level,n_features,n_trees):
    ...
    #PARA 3 CLASES
    data= {'fscore': ['General', 'Stage 2','Slow wave stage']}

    df = pd.DataFrame(data)
    df.to_csv('results_cascade_S2-sws.csv')
            ...
            for l in level:
                for f in n_features:
                    ...
                    #preparamos archivo csv para almacenar datos
                        ...
                        #     (acc[i], acc_s2[i], acc_sws[i],
                        #     f1[i], f1_s2[i], f1_sws[i]) = train_data(w, r, l, f, t, copy.deepcopy(train_epochs),i)             
                        # save_results(f1,f1_s2,f1_sws, w,r,l,f,t)
