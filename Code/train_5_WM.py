...
def save(data, path, labels):
    np.savetxt('features_eeomg.csv', data, delimiter=',')
    np.savetxt('labels_eeomg.csv', labels)
...
def train_data(wavelet, rus, level, features, trees, epochs, index):
    ...
    file = open("Fscore_MCM_eeomg.txt", "w")
...
def save_results(f1, f1_w, f1_s1, f1_s2, f1_sws, f1_r, w, r, l, f,t):
    ...
    df = pd.read_csv("results_MCM_eeomg.csv")
    f1= np.mean(f1)
    ...
    df.to_csv("results_MCM_eeomg.csv", index=False)
...
def predecir_sj(wavelet, rus, level, features, rf, test, medias, stds,pca,index):
    ...
    data.to_csv('test_subject_' + str(index) + '.csv', index=False)
    ...
    plt.savefig('confusion_matrix_eeomg_'+ str(index)+'.png')  # doctest: +SKIP
...
def main(wavelet,RUS,level,n_features,n_trees):
    ...
    df.to_csv('results_MCM_eeomg.csv')
