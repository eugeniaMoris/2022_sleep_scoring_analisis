...
def save(data, path, labels):
    np.savetxt('features_eeog.csv', data, delimiter=',')
    np.savetxt('labels_eeog.csv', labels)
...
def wavelet_level(señal, level,wavelet, valid, medias, stds):
    ...
    for e in señal:
        ...
        Features = np.append(CA_EEG, CA_EOG)
        #CA = np.append(CA, CA_EMG)
        #CA= CA_EEG
...
def train_data(wavelet, rus, level, features, trees, epochs, index):
    ...
    file = open("Fscore_MCM_eeog.txt", "w")
...
def save_results(f1, f1_w, f1_s1, f1_s2, f1_sws, f1_r, w, r, l, f,t):
    ...
    df = pd.read_csv("results_MCM_eeog.csv")
    f1= np.mean(f1)
    ...
    df.to_csv("results_MCM_eeog.csv", index=False)
...
def predecir_sj(wavelet, rus, level, features, rf, test, medias, stds,pca=10):
    ...
    plt.savefig('confusion_matrix_eeog')  # doctest: +SKIP
...
def main(wavelet,RUS,level,n_features,n_trees):
    ...
    df.to_csv('results_MCM_eeog.csv')
