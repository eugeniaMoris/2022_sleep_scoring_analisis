...
def save(data, path, labels):
    np.savetxt('features_EM.csv', data, delimiter=',')
    np.savetxt('labels_EM.csv', labels)
...
def save_results(f1, f1_w, f1_s1, f1_s2, f1_sws, f1_r, w, r, l, f,t):
    ...
    df = pd.read_csv("results_MCM_EM.csv")
    f1= np.mean(f1)
    ...
    df.to_csv("results_MCM_EM.csv", index=False)
...
def predecir_sj(wavelet, rus, level, features, rf, test, medias, stds,pca,index):
    ...
    plt.savefig('confusion_matrix_EM_'+ str(index) +'.png')  # doctest: +SKIP
