...
def train_data(epochs, index, subjects, modelo, wavelet, n_features, rus, n_trees):
    ...
    if modelo == 1:
        first_labels[first_labels==3]=4 #segundo codelo
    ...
    if modelo == 1:
        y_valid[y_valid==3]=4 #para entrenar segundo modelos
...
def predict_test(model1,model2,test,media1,media2,std1,std2,pca1,pca2,w1,w2):
    ...
    for t in test:
        ...
        #MODELO 2
        w1CA, w1CD1, w1CD2 = pywt.wavedec(t[0,:],w1,level=2)
        w2CA, w2CD1, w2CD2, w2CD3 = pywt.wavedec(t[0,:],w2,level=3)

        CAM1 = np.append(w1CA,w1CD1)
        CAM1 = np.append(CAM1,w1CD2)
        CAM2 = np.append(w2CA,w2CD2)
    ...
    for i in range(len(y_predict_1)):
        if y_predict_1[i]==2:
            y_predict[i]=2
        if (y_predict_1[i] == 4) and (y_predict_2[i] == 3):
            y_predict[i]=3
