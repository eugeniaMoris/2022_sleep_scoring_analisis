...
def save(data, path, labels):
    np.savetxt('features_test.csv', data, delimiter=',')
    np.savetxt('labels_test.csv', labels)
...
def train_data(wavelet, rus, level, features, trees, epochs, index):
    ...
    file = open("Fscore_age_test.txt", "w")
...
def save_results(f1, f1_w, f1_s1, f1_s2, f1_sws, f1_r, w, r, l, f,t):
    ...
    df = pd.read_csv("test_results_AGE.csv")
    f1= np.mean(f1)
    ...
    df.to_csv("test_results_AGE.csv", index=False)
...
def train(wavelet, rus, level, features,trees, epochs, test_epochs):
    ...
    f = open("Acc_MCM_test.txt", "w")
    f.write('Data: '+ 'acc: '+ str(np.mean(acc)) +'acc_w '+ str(np.mean(acc_w))+'acc_s1 '+ str(np.mean(acc_s1))+'acc_s2 '+ str(np.mean(acc_s2))+'acc_sws '+ str(np.mean(acc_sws))+'acc_r '+ str(np.mean(acc_r)))
    f.close()

    file = open("Fscore_MCM_test.txt", "w")
...
def predecir_sj(wavelet, rus, level, features, rf, test, medias, stds,pca=10):
    ...
    plt.savefig('test_confusion_matrix')  # doctest: +SKIP
...
def main(wavelet,RUS,level,n_features,n_trees):
    ...
    epochs,train_sj = read_EDF_data_per_sj_5.get_eeg_data(10,40,0,'early')
    #print('Train epochs len: ', len(epochs))
    test_epochs,_ = read_EDF_data_per_sj_5.get_eeg_data(10,40,0,'early')
    ...
    cant_test = 70 * len(test_epochs) / 100
    cant_test= int(cant_test)

    #70 entrenamiento 30 test
    train_epochs = epochs[:cant_train]
    #test_epochs = epochs[cant_train:]
    test_epochs = test_epochs[cant_test:]
    ...
    df.to_csv('test_results_AGE.csv')
