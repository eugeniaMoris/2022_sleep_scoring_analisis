...
def save(data, path, labels):
    np.savetxt('features_eeg.csv', data, delimiter=',')
    np.savetxt('labels_eeg.csv', labels)
...
def train_data(wavelet, rus, level, features, trees, epochs, index):
    ...
    file = open("Fscore_MCM_eeg.txt", "w")
...
def save_results(f1, f1_w, f1_s1, f1_s2, f1_sws, f1_r, w, r, l, f,t):
    ...
    df = pd.read_csv("results_MCM_eeg.csv")
    f1= np.mean(f1)
    ...
    df.to_csv("results_MCM_eeg.csv", index=False)
...
def predecir_sj(wavelet, rus, level, features, rf, test, medias, stds,pca=10):
    ...
    plt.savefig('confusion_matrix_eeg')  # doctest: +SKIP
